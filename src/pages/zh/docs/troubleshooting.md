...
Mobile SSH 会在自动更正与预测建议均已关闭的情况下将按键直接发送到 shell，因此键盘不应在文本到达远程端之前改写它。如果你的键盘仍然改动了输入，请确认没有系统级的替换或剪贴板工具在拦截它，并使用扩展按键栏输入 `ESC`、`TAB`、`CTRL`、方向键、`HOME`、`END`、`PGUP` 和 `PGDN` 等终端按键。
