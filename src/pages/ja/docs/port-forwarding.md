...
title: "ポートフォワーディング"
description: "Mobile SSH のローカルポートフォワーディングの構文、IPv6 の宛先、Android および iOS で保存済みトンネルが立ち上がる仕組み。"
---
...
転送は保存済みサーバー側で宣言し、そのサーバーに接続したときに立ち上がります。セッションの途中でトンネルを追加・削除する専用の画面は、どちらのプラットフォームにもありません。転送を変更するには、サーバープロファイルを編集して接続し直してください。

これは意図的なトレードオフです。プロファイルに属するトンネルは、接続するたび — 新しいネットワークでの再接続後も含めて — 手で組み直さなくても戻ってきます。
