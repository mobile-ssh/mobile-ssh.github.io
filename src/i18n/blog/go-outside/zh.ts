...
export const zh = defineGoOutside({
  ...
  body: [
    ...
    `平台差异是真实存在的，值得提前规划。在 Android 上，应用退到后台时由前台服务保持连接——那条常驻通知<em>就是</em>会话。iOS 只允许应用在后台短暂运行；暂停后，等你回来时，它会重新连接并重新挂载到多路复用器。提醒需要连接保持活跃。无论哪种情况，服务器上的智能体都不曾停下，这才是关键。关上的只是那扇窗。`,
    `Aloha 不是马虎的借口`,
  ...
  truth: {
    ...
    rows: [
      ...
      { item: "iOS 上的连接", value: "后台短暂运行，然后重新连接", survives: false },
      { item: "没有信号可以投递的提醒", value: "需要有效连接；不保证送达", survives: false }
    ],
