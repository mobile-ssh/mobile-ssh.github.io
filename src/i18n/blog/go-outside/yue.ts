...
export const yue = defineGoOutside({
  ...
  body: [
    ...
    `平台差異係真嘅，值得預先計劃。喺 Android，個 app 退到背景嗰陣由一個前景服務保持住條連線——嗰條常駐通知<em>就係</em>個 session。iOS 只畀個 app 喺背景行一陣；暫停之後，你返嚟嗰陣佢會重新連線，再 attach 返你個 multiplexer。要收到提醒，條連線就要保持運作。無論邊種情況，伺服器上面個代理都冇停過，呢樣先係重點。閂咗嘅只係扇窗。`,
    `Aloha 唔係求其做嘢嘅藉口`,
  ...
  truth: {
    ...
    rows: [
      ...
      { item: "iOS 上面條連線", value: "喺背景行一陣，之後重新連線", survives: false },
      { item: "冇訊號送出嘅提示", value: "要有連線先得；唔保證一定收到", survives: false }
    ],
