...
export const ja: Dict = {
  ...
  compare: {
    ...
    agentAppsSourcesNote: "確認日：2026-09-19。Moshi の直接接続 SSH 端末と、エージェントのイベントや承認を処理する任意のバックエンドは別です。Android のストア掲載情報では SFTP 対応をうたっていますが、詳細なドキュメントでは SCP によるアップロードと開発サーバーのプレビューを説明しているため、該当する表の項目には条件を明記しています。Orca のモバイルアプリはパソコン上の Orca とペアリングします。LAN/Tailscale では Orca アカウントなしで利用できますが、Relay にはアカウントが必要です。デスクトップ版限定の機能や、ドキュメントに記載のない機能は表内に明記しています。",
  },
  privacy: {
