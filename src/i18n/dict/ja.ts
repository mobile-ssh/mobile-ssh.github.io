...
export const ja: Dict = {
  ...
  home: {
    ...
    metaDescription:
      "Mobile SSH は、マルチセッション ターミナル、Eternal Terminal の耐障害性セッション、tmux マネージャー、SFTP 転送、秘密鍵、ポートフォワーディング、プラグインを備えた、Android と iOS 向けの絞り込まれた SSH クライアントです。",
    eyebrow: "Android・iOS 用 SSH クライアント",
    h1: "Mobile SSH",
    intro:
      "Android と iOS 向けの絞り込まれた SSH クライアント。マルチセッション ターミナル、ネットワーク切断を乗り越える Eternal Terminal セッション、内蔵 tmux マネージャー、SFTP ファイル転送、ローカル ポートフォワーディング、プラグインをスマートフォンとタブレットのために用意しました。",
    ...
    videoIosAlt: "iOS のデモ：Mobile SSH を開いてライブの SSH ターミナル セッションに入り、「最近」やその他の画面があるホーム画面に戻るまで",
    ...
    demoItems: [
      {
        ...
        text: "Android は接続をフォアグラウンド サービスで保持し、進行中の通知そのものがその接続です — 接続先のサーバー名を表示し、「すべて切断」を提供します。ログが流れている最中にホーム画面へ抜けて戻ってきても、止まってなどいません。iOS にはバックグラウンド実行がないため、代わりに再接続して tmux 自動アタッチでつなぎ直します。",
        androidAlt:
  ...
  features: {
    ...
    metaDescription:
      "Android と iOS の SSH：ターミナル、Eternal Terminal、tmux マネージャー、SFTP、秘密鍵、ポートフォワーディング、プラグインに関する Mobile SSH の主な機能。",
    eyebrow: "機能一覧",
    h1: "Mobile SSH の機能",
    intro:
      "Mobile SSH は Android と iOS の端末から直接 SSH を使うために作られています。ターミナルを開き、生かし続け、ファイルを動かし、サーバーを保存し、モバイルの中断からすばやく復帰します。",
    groups: [
      {
        title: "プラットフォーム",
        items: [
      ...
      },
      {
        ...
        items: [
          ...
          "tmux コマンド追跡と中断した作業向けの再アタッチのヒント。接続時に何へアタッチするかはサーバーごとに選択可能：自動判別、なし、tmux、herdr、Zellij（Android）",
  ...
  compare: {
    ...
    rows: [
      {
        need: "主な用途",
        mobileSsh: "Android と iOS 向けの SSH、SFTP、ローカル トンネル、ターミナルの専用クライアント。",
      ...
      {
        need: "モバイルのターミナル操作",
        mobileSsh: "追加キーの行、グリッド セッション、フルスクリーン ペイン、ピンチ ズーム、コピー／共有付き選択、tmux に配慮したスクロール — 両プラットフォームでのネイティブなパススルー入力と外付けキーボード対応付き。",
        termux: "強力なターミナル環境。動作は入れたツールと設定次第。",
        termius: "モバイル キーボード アドオン、ジェスチャー、タブ、自動補完、スニペット、洗練されたターミナル UX。"
      },
      {
        need: "ファイル転送",
        mobileSsh: "アクティブな SSH セッションに紐づく内蔵のデュアルペイン SFTP 転送。",
    ...
    agentAppsRows: [
      { feature: "スマートフォンからコーディング エージェントを実行",             mobile: "yes", onepilot: "yes",      happy: "yes",        omnara: "yes",     chatgpt: "yes",          claude: "yes" },
      ...
      { feature: "ローカル ポートフォワーディング",                             mobile: "yes", onepilot: "yes",      happy: "no",         omnara: "no",      chatgpt: "no",           claude: "no" },
  ...
  privacy: {
    ...
    metaDescription:
      "ローカル保存、SSH データ送信、ファイル転送、ログ、Android と iOS のアプリの権限を含む Mobile SSH のプライバシーポリシー。",
    eyebrow: "プライバシー",
    h1: "プライバシーポリシー",
    intro:
    ...
    sections: [
      {
        heading: "端末に保存される情報",
        body: "アプリのデータを保存することを選んだ場合、Mobile SSH はそれを端末にローカル保存します。シークレットは iOS ではシステムの Keychain に保管され、Android では端末外に持ち出せない Android Keystore 内の鍵で暗号化されます。また、アプリは Android のクラウド バックアップから除外されています。保存されるものには、サーバープロファイル、ユーザー名、ポート、パスワード、秘密鍵、鍵のパスフレーズ、ポートフォワーディングのルール、最近のセッション、ログイン試行履歴、ファイル転送のパス、並べ替え設定、tmux セッションのスナップショットと再アタッチのヒント、アプリ設定、デバッグ記録が有効な場合のデバッグログが含まれます。"
      },
      {
        heading: "ネットワーク経由で送信される情報",
        body: "Mobile SSH は SSH 認証データを、ユーザーが設定して接続するサーバーにのみ送信します。ターミナルの入出力、SFTP ファイルの内容、ローカル ポートフォワーディングのトラフィックは、ユーザーが選んだサーバーやリモート エンドポイントとの間でやり取りされます。Mobile SSH の分析、広告、テレメトリ、クラウド同期サービスにこれらのデータを送信することはありません。"
      },
      {
        heading: "ファイル転送とストレージ アクセス",
        body: "ファイル転送機能は、ファイルをアップロード／ダウンロードするために端末ローカルのストレージとリモートの SFTP ディレクトリを参照します。Mobile SSH は Android に対して包括的なストレージ権限を要求しません。システムのフォルダー選択ツールで 1 つのフォルダーを選ぶと、アプリはその中だけを読み書きできます。iOS では、ローカルのファイルや写真にはシステムのドキュメント選択ツールと写真選択ツールを通じてアクセスします。"
      },
      {
        heading: "ログとトラブルシューティング",
        body: "ログイン履歴と任意のデバッグログはトラブルシューティングのためにローカル保存され、どちらも自分でオンにするまでは無効か空のままです。Android のデバッグ記録は、ターミナルのイベント、SSH データのサイズ、タッチ入力の診断、リサイズ イベント、トンネルのライフサイクル イベントを取得します。記録の開始前に、入力したすべてのキー（パスワードを含む）が記録される旨の警告が表示され、停止するとアーカイブが Downloads フォルダーに書き出されます。iOS はこれとは別の、より範囲の狭いログを記録します。接続を試みたアドレスとそれぞれの失敗理由、再接続とバックオフ、切断、ネットワークの変化、tmux コマンドとそのエラーです。デバッグログやアーカイブをサポートや他者に共有する前に内容を確認してください。"
      },
    ...
    permissions: [
      ...
      { label: "ファイル アクセス", body: "Android ではシステムのフォルダー選択ツールでフォルダーごとに、iOS ではシステムのドキュメント選択ツールと写真選択ツールを通じて許可されます。Mobile SSH はどちらのプラットフォームでも包括的なストレージ権限を要求しません。" }
    ],
  ...
  docsIndex: {
    ...
    intro:
      "Mobile SSH のインストール、接続、セッション管理、ファイル転送、ポートフォワーディング、トラブルシューティングの公開ガイドとしてこれらのページをご利用ください。",
    cards: [
      { slug: "getting-started",  title: "はじめに",               text: "インストール、アプリの起動、最初のサーバーへの接続、よく使うホストの保存。" },
      { slug: "terminal",         title: "ターミナル",             text: "ペイン、追加キーの行、スクロール、コピー操作、tmux 動作、キーボード設定。" },
      { slug: "file-transfer",    title: "ファイル転送",           text: "端末とサーバーのファイル閲覧、アップロード、ダウンロード、並べ替え、リモート詳細の確認。" },
      { slug: "port-forwarding",  title: "ポートフォワーディング", text: "ローカル トンネル文字列の記述、IPv6 宛先への転送、接続時の自動起動。" },
  ...
  about: {
    ...
    eyebrow: "情報",
    h1: "Mobile SSH について",
    intro: "tmux、ポートフォワーディング、SFTP に対応するマルチセッション SSH クライアント。",
    appHeading: "アプリについて",
    versionLabel: "バージョン",
    authorLabel: "作者",
