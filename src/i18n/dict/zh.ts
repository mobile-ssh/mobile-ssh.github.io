...
export const zh: Dict = {
  ...
  home: {
    ...
    demoItems: [
      {
        title: "会话比应用活得更久",
        text: "Android 把连接放在前台服务里，那条常驻通知就是这个连接——它写着服务器的名字，并提供「全部断开」。日志正在滚动时切回主屏幕，回来时会发现它从未中断过。iOS 没有后台执行，取而代之的是重新连接并用 tmux 自动附加。",
        androidAlt:
          "Android 演示：会话在后台持续运行——终端里实时滚动的 HTTP 日志、通知权限提示、写着已连接服务器名字并带「全部断开」的常驻通知，然后切回主屏幕再返回，同一条日志流仍在源源不断地到达。"
      },
      {
        title: "密钥在这里生成，而不是敲进来",
        text: "在手机上生成一把 Ed25519 密钥：一个标签、一个用户名、一个可选的密码短语。随后弹出的表单会显示公钥部分，供你添加到服务器的 authorized_keys；公钥和私钥都可以分享或保存。",
        androidAlt:
          "Android 演示：生成密钥——「生成 SSH 密钥」对话框里的标签、用户名、Ed25519 密钥类型和密码短语，然后是提供分享或保存密钥的公钥表单。"
      },
      {
