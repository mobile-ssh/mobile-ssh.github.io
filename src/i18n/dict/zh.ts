...
export const zh: Dict = {
  ...
  home: {
    ...
    demoItems: [
      {
        title: "会话比应用活得更久",
        text: "Android 把连接放在前台服务里，那条常驻通知就是这个连接——它写着服务器的名字，并提供「全部断开」。日志正在滚动时切回主界面，回来时会发现它从未中断过。iOS 没有后台执行，取而代之的是重新连接并用 tmux 自动附加。",
        androidAlt:
    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          "可在服务器配置中选择的可复用凭据记录",
          "围绕「继续未完成的工作」设计的主界面：当前活动的连接，以及在你的服务器上等待的 tmux 会话——后者取自本地保存的快照，因此没有网络也能列出，每一行都标注了快照的新旧程度。iOS 上还保留「最近」列表",
      ...
      },
      {
        ...
        items: [
          ...
          "tmux 命令跟踪与中断作业的重新附加提示，并可按服务器选择连接时附加什么：自动检测、不附加、tmux、herdr 或 Zellij（Android）",
      ...
      },
      {
        ...
        items: [
          ...
          "在 Android 上，文件传输会回到该 tmux 会话上次所在的目录，并在窗格标题中标出会话名；没有可记忆的位置时，则退回到你在该主机上最常用的目录",
      ...
      },
      {
        ...
        items: [
          ...
          "将已保存的服务器和凭据导出为备份文件——可以全部导出，也可以只勾选你需要的条目；点按文件夹标题即可选中整个文件夹",
          "可选口令会加密备份；导入时可合并或替换",
  ...
  privacy: {
    ...
    sections: [
      {
        heading: "存储在你设备上的信息",
        body: "如果你选择保存应用数据，Mobile SSH 会将其保存在设备本地。在 iOS 上，机密保存在系统 Keychain 中；在 Android 上，机密由一把存放在 Android Keystore、无法导出到设备之外的密钥加密，并且应用已选择退出 Android 云备份。这可能包括保存的服务器配置、用户名、端口、密码、私钥、密钥口令、端口转发规则、最近会话、登录尝试历史、文件传输路径、文件排序偏好、tmux 会话快照与重新附加提示、应用设置，以及在启用调试录制时的调试日志。"
      },
      {
        heading: "通过网络发送的信息",
      ...
      {
        heading: "日志与故障排查",
        body: "登录历史与可选的调试日志会在本地保存以便排错，两者在你主动开启之前都处于关闭或为空的状态。Android 的调试录制会记录终端事件、SSH 数据大小、触摸输入诊断、尺寸调整事件以及隧道生命周期事件——开始录制前它会先警告你，这将包含你敲下的每一个按键（密码也在内），并会把存档写入你的「下载」文件夹。iOS 记录的是另一份范围更窄的日志：尝试过的地址与各自失败的原因、重连与退避、连接断开、网络变化，以及 tmux 命令及其错误。将任何调试日志或存档分享给支持人员或他人之前，请先检查内容。"
      },
  ...
  docsIndex: {
    ...
    cards: [
      ...
      { slug: "terminal",         title: "终端",           text: "窗格、扩展按键栏、滚动、复制操作、tmux 行为与键盘设置。" },
