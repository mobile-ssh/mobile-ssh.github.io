...
export const zh: Dict = {
  ...
  compare: {
    ...
    agentAppsRows: [
      ...
      { feature: "代理提醒（已连接的代理需要输入时）",       mobile: "yes", onepilot: "yes",      happy: "yes",         omnara: "yes",     chatgpt: "yes",          claude: "yes" },
  ...
  privacy: {
    ...
    sections: [
      ...
      { heading: "匿名使用分析", body: "Mobile SSH 通过 HTTPS 向 Aptabase 发送匿名应用和功能使用事件，包含应用及系统版本、设备型号和语言。使用定期轮换的随机会话标识，绝不包含服务器地址、用户名、凭据、命令或文件内容。分析默认开启，两个平台均可在设置停止收集新事件；iOS 之前已排队的事件仍可能发出。" },
      {
        "heading": "导出的备份",
